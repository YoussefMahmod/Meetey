...
import time
# webdriver class will connect us to a browser’s instanceFor Browser connection
from selenium import webdriver
# emulate the stroke of keyboard keys, including special keys like “Shift” and “Return”.
...
def add_meet():
    if error_message():
        return
    try:
        db.insert(meet_name_text.get(), start_date_text.get(),
                  end_date_text.get(), meet_link_text.get())
    except sqlite3.IntegrityError:
        messagebox.showerror('Meeting name exists!',
                             'Meeting name should be unique.')
    fetch_data()
...
def update_data():
    if error_message() == 1:
        return
    db.update(selected_item[0], meet_name_text.get(
    ), start_date_text.get(), end_date_text.get(), meet_link_text.get())
    fetch_data()
...
# def start_app():
#     app_working = True
#     status_label.config(fg='green', text='Working', font=('bold', 14))


# def stop_app():
#     app_working = False
#     status_label.config(fg='red', text='Not Working', font=('bold', 14))
...
data_list.configure(bg='#232d3b', fg='white', highlightbackground='green',
                    borderwidth=0, font=('Courier New', 16))
...
# Buttons
add_meet_btn = Button(app, text='Add Meet', width=12,
                      pady=10, command=add_meet)
add_meet_btn.grid(row=4, column=0)
add_meet_btn.configure(bg='#121417', fg=btn_color,
                       highlightbackground='green', borderwidth=.05)
update_btn = Button(app, text='Update', width=12, pady=10, command=update_data)
update_btn.grid(row=4, column=1)
update_btn.configure(bg='#121417', fg=btn_color,
                     highlightbackground='green', borderwidth=.05)
remove_btn = Button(app, text='Delete', width=12, pady=10, command=delete_data)
remove_btn.grid(row=4, column=2)
remove_btn.configure(bg='#121417', fg=btn_color,
                     highlightbackground='green', borderwidth=.05)
clear_btn = Button(app, text='Clear Fields', width=12,
                   pady=10, command=clear_data)
clear_btn.grid(row=0, column=2)
clear_btn.configure(bg='#121417', fg=btn_color,
                    highlightbackground='green', borderwidth=.05)
# start_btn = Button(app, text='Start', width=12, pady=10, command=start_app)
# start_btn.grid(row=12, column=0)
# start_btn.configure(bg='#121417', fg=btn_color,
#                     highlightbackground='green', borderwidth=.05)
# stop_btn = Button(app, text='Stop', width=12, pady=10, command=stop_app)
# stop_btn.grid(row=12, column=1)
# stop_btn.configure(bg='#121417', fg=btn_color,
#                    highlightbackground='green', borderwidth=.05)

# Status label
# status_label = Label(app, text='Not Working', font=('bold', 14), pady=20)
# status_label.configure(bg='#121417', fg='red')
# status_label.grid(row=12, column=2, sticky=W)
...
# s_time_check_btn.configure(bg='#121417',fg=btn_color,highlightbackground='g   reen',borderwidth=.05)
...
def check_time():
    ...
    print(f"{h}:{m}")
    rows = db.fetch_start_date(f"{h}:{m}")
    print(f"from check time {rows}")
    if rows:
        # Open class link in google chrome
        driver.get(rows[0][0])
        # Press Join Button if exist
        try:
            join_btn = driver.find_element_by_class_name(
